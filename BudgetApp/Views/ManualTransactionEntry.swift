...
struct ManualTransactionEntry: View {
    ...
    private func saveTransaction() {
        ...
        // Categorize with smart service
        Task {
            let enhancedTransaction = await transactionManager.smartService.categorizeTransactions([transaction])
            await MainActor.run {
                if let enhanced = enhancedTransaction.first {
                    if let index = transactionManager.transactions.firstIndex(where: { $0.id == transaction.id }) {
                        transactionManager.transactions[index] = enhanced
                    }
                }
            }
        }
